...
class Bank(User):
    ...
    def withdraws(self):
        dp = float(input(f"{self.name.title()}, please enter how much you would like to withdraw"))
        if self.balance < wd:
...
def options(user_two):
    print("Thank you fro creating your bank account")
    ...
    while True:
        ...
        if option_choice == 1:
            ...
            if option_choice == 1 and user_two != None:
                ...
                if option__choce == 5 and user_two != None:
                    rprint(f"There have been {user_two_bank.total_deposits} deposits.")
...
while True:
    ...
    if new.user.lower() == 'yes':
        name = input("Enter the second person's name: ")
        age = int("Enter the second person's age: ") 
        ...
        user_two_balance = bank_creation(user_tw.name)
